...
# Этот класс сделан для того чтобы в классах - CashCalculator, CaloriesCalculator не повторялся одинаковый функционал
class Calculator:
	...
	# Здесь подсчитываем все количество затрат или калорий, которые находились в amount 
	def get_today_stats(self):
		total = 0
		
		# Перебираем весь список 
		for record in self.records:   
			# Эта функция(метод) отвечает за то что бы записывать все затраты сегодня, 
			# так что тут мы берём только те записи которые с сегодняшней датой 
	...
	# Тут все практически то же самое как в функции(методи) get_today_stats(), только все количество затрат или калорий за неделю
	def get_week_stats(self):
		# Здесь охватываем период за неделю
		during_week = dt.datetime.now() - dt.timedelta(days=7)
		total = 0
		
		# Тут все то же самое как в том методе только еще записываем даты которые на этой неделе
		for record in self.records:     
...
class CashCalculator(Calculator):
	# Сurrency это курс в котором хочет его увидеть пользователь, доступные значения - 'rub', 'eur', 'usd'. 
	def get_today_cash_remained(self, currency):
		...
		if currency == 'rub':
			...
			# Это переменная лишь нужна, что бы потом в сообщение выводить правильно - N руб/USD/Euro
			currency = 'руб'

		elif currency == 'usd':
			# Тут отнимаем от лимита все что было потрачено за сегодня, и приводим к евро
			...
			# Тут отнимаем от лимита все что было потрачено за сегодня, и приводим к долларам
		...
		if ostatok > 0:
			# Тут выводим остаток уже в определённом курсе и currency = переменная в которою выше положили названия курса
...
class CaloriesCalculator(Calculator):    
	# Тут практически все то же самое как в классе CashCalculator только без всяких курсов и выводится все про калории 
...
# Тут для удобства сделан класс который распределяет по свойствам данные про деньги или калории
class Record:
	...
	def __init__(self, amount, comment, date = ''):
		...
		# Здесь говорится что нужно если в date ничего не укажут ставить сегодняшнею дату
		if date == '':
